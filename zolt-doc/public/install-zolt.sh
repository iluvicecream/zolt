...
case "$os" in
    ...
    *)
        echo "error: unsupported OS '$os' (for windows please download directly from github releases)" >&2
        exit 1
        ;;
esac
...
case "$arch" in
    ...
    *)
        echo "error: unsupported architecture '$arch' (zolt ships binaries for amd64 and arm64)" >&2
        exit 1
        ;;
esac

asset="zoltd-${os_asset}-${arch_asset}"
