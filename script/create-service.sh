...
usage() {
    echo "usage: $0 <ssh-host> <working-path> <zoltd-binary-path> <service-name>"
    echo "example: $0 user@example.com /home/myapp/ /home/myapp/zoltd my-service"
}
...
out=$(ssh -o ConnectTimeout=10 "$host" "test -f '$bin_path' && echo FILES_OK" || true)
if [[ "$out" != *FILES_OK* ]]; then
    echo "error: config or binary not found on $host ($bin_path)" >&2
    exit 1
fi
